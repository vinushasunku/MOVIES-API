...
BEGIN
  ...
  IF @Type ='AvgRating'
  BEGIN
	  SELECT top 5 m.MovieID,m.Title,m.YearReleased,m.GenreType,ROUND(AVG(wh.Rating) * 2, 0) / 2   as AverageRating,wh.Rating FROM Movies m 
	  LEFT JOIN WatchHistory wh ON wh.MovieID=m.MovieID
	  GROUP BY m.MovieID,m.Title,m.YearReleased,m.GenreType,wh.Rating
  ...
  BEGIN
	  ...
	  SELECT top 5 m.MovieID,m.Title,m.YearReleased,m.GenreType, wh.Rating,UI.UserID FROM Movies m 
	  LEFT JOIN WatchHistory wh ON wh.MovieID=m.MovieID
